...
def action_assign():
    ...
    if request.vars.notifset == 'set_bulk':
        db.notifications.insert(notif_use='Gnrl_leads', lead_id=request.vars.lead_no, Message='New {} General datas assigned'.format(request.vars.notifset_no), Given_by=auth.user_id, Given_to=request.vars.assign_to)
        db.commit()
